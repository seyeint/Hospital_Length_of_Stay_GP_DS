...
def main():
    ...
    # THESE ARE OUR BEST PARAMETERS. It also uses phenotypic fitness sharing.
    ...
    sr7 = SymbolicRegressor(population_size=250,
                            ...
                            edv_stopping_criteria=0.0, n_semantic_neighbors=20,
    ...
    sr7.fit(X_train, y_train)
    y_test_pred = sr7.predict(X_test)
    mae = mean_absolute_error(y_test, y_test_pred)
    mse = mean_squared_error(y_test, y_test_pred)
    rmse = np.sqrt(mse)  # no need to print it
    #
    rsquared = 1 - (mse / base_mse)
    print('Model {}: \n mae: {} \n mse: {} \n r2: {} \n'.format(sr7, mae, mse, rsquared))
    scores.append((sr7, [mae, mse, rsquared]))
    ...
    graph = sr7._program.export_graphviz()
    graph_str = str(graph)
    program_str = str(sr7._program)
