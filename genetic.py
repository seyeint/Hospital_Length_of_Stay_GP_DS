...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, train_indices, val_indices, seeds, params):
    ...
    def _tournament():
        ...
        if metric.greater_is_better:
            ...
            winner_fitness = parents[np.argmax(fitness)].fitness_
        else:
            parent_index = contenders[np.argmin(fitness)]
            winner_fitness = parents[np.argmin(fitness)].fitness_

        return parents[parent_index], parent_index, winner_fitness

    def _disassortative_selection():  # this implementation was normal with a selection gathering 2 members and not one, twice
        pheno_dif_rate=0.1
        """Find the fittest individual from a sub-population."""
        contenders = random_state.randint(0, len(parents), tournament_size)
        fitness = [parents[p].fitness_ for p in contenders]
        if metric.greater_is_better:
            ...
            fitness_s = sorted(range(len(fitness)), key=lambda k: fitness[k])
            best_contender=contenders[fitness_s[0]]
            max_dif=pheno_dif_rate*best_contender.fitness_
            for index in range(1,tournament_size):
                if (abs(best_contender.fitness_ - contenders[fitness_s[index]].fitness_)> max_dif):
                    parent_index = contenders[np.argmin(fitness)]

        return parents[parent_index], parent_index,winner_fitness

    def _elitism():
        contenders = random_state.randint(0, len(parents), tournament_size)  # tournament size
        fitness = [parents[p].fitness_ for p in contenders]
        selection_probs = []
        sum=0
        ...
        if metric.greater_is_better:
            fitness_s = sorted(range(len(fitness)), key=lambda k: fitness[k])
            for x in range(len(fitness_s)):
                selection_probs.append(x/sum)
            result = npr.choice(len(fitness), p=selection_probs)
            parent_index = fitness_s[result] #to get the index of the contender
        else:
            winner_fitness = parents[np.argmin(fitness)].fitness_

            fitness_s = sorted(range(len(fitness)), key=lambda k: fitness[k], reverse=True)
            for x in range(len(fitness_s)):
                selection_probs.append(x / sum)
            result = npr.choice(len(fitness), p=selection_probs)
            parent_index = fitness_s[result]  # to get the index of the contender
    ...
    def _roulletewheel():
        ...
        max = sum(fitness)
        if metric.greater_is_better:
            selection_probs = [x/max for x in fitness]
            parent_index= npr.choice(len(fitness), p=selection_probs)
        else:
            winner_fitness = parents[np.argmin(fitness)].fitness_

            selection_probs = [x/max for x in fitness]
            selection_probs = list(reversed(selection_probs))
            parent_index= npr.choice(len(fitness), p=selection_probs)
        return parents[parent_index], parent_index, winner_fitness
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            parent, parent_index, winner_fitness = _tournament()

            if method < method_probs[0]:
                # GP: swap crossover
                donor, donor_index, winner_fitness = _tournament()
                ...
                if semantical_computation:
                    program = parent.gs_crossover_semantics(X,y,sample_weight,train_indices, donor, random_state) #STUFF WE CHANGED TO BE ABLE TO GET OUR FITNESS
                else:
                    program = parent.gs_crossover(donor.program,winner_fitness, random_state)
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            def phenotypic_fitness_sharing(index):
                minDist = 0.05
                shareParam = 0.5
                denominator = 1
                for x in range(len(population)):
                    dist = abs(population[index].raw_fitness_ - population[x].raw_fitness_)
                    if (dist < minDist):
                        denominator += (1 - (dist / shareParam))
                        population[index].raw_fitness_ = population[index].raw_fitness_/ denominator

            def depth_fitness_sharing(index):
                minDist = 1
                shareParam = 0.1
                denominator = 1
                for x in range(len(population)):
                    dist = abs(population[index].length_ - population[x].length_)
                    if (dist <minDist): #to try and explore better each level of depth
                        denominator += (1 - (dist / shareParam))
                        population[index].raw_fitness_ = population[index].raw_fitness_ * denominator

            for x in range(len(parents)):
                phenotypic_fitness_sharing(x)
                #depth_fitness_sharing(x)
                #pass
